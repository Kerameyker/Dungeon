...
namespace Hollow
{
    ...
    public partial class Game
    {
        ...
        void TryStory(StoryTrigger trigger)
        {
            var beat = StoryCatalog.Next(trigger, Floor, MaxFloor, Flags);
            if (beat == null) return;
        ...
        {
            if (Flags.Has(beat.Id)) return;
            Flags.Add(beat.Id);
            PlayDialogue(beat.Lines, () =>
            {
                ApplyEffect(beat.Effect);
                Save();
                TryStory(trigger);   // chained scenes that unlock right after this one
            });
        }
        ...
        IEnumerator StoryAfterBoss(int floor)
        {
            ...
            if (Floor != floor || Player == null || Player.IsDead) yield break;
            TryStory(StoryTrigger.BossKilled);
        }
