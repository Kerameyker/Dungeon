...
namespace Hollow
{
    ...
    public partial class Game
    {
        ...
        void UpdateMenu()
        {
            ...
            if (!GameInput.UsingGamepad)
            {
                ...
                if (hv >= 0 && hv + _scroll < n)
                {
                    ...
                    if (GameInput.MenuClickPressed) { _cursor = abs; activate = true; }
                }
            }
        ...
        void BuildShop()
        {
            ...
            if (!_shopSell)
            {
                ...
                AddRow("Random gear  -  " + price + " g", ItemIcons.Consumable(ItemIcons.ConsumableIcon.Gold), Color.clear, p.Gold < price,
                       "A random Rare-or-better piece scaled to your highest floor.\nCould be any slot and any weapon type.", () => BuyGear(price));
            }
            else
            {
        ...
        void ClaimQuest(string id)
        {
            ...
            if (def.RewardPotions > 0) Player.Supplies.AddPotion(def.RewardPotions);
            _notified.Remove(id);
